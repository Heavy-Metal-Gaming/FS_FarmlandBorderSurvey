---
--- PropertyBordersSettingsEvent.lua
--- Network event for synchronizing property border settings in multiplayer.
---
--- Sent from server to clients (and client to server) whenever settings change.
---

PropertyBordersSettingsEvent = {}
local PropertyBordersSettingsEvent_mt = Class(PropertyBordersSettingsEvent, Event)

InitEventClass(PropertyBordersSettingsEvent, "PropertyBordersSettingsEvent")

function PropertyBordersSettingsEvent.emptyNew()
    local self = Event.new(PropertyBordersSettingsEvent_mt)
    return self
end

function PropertyBordersSettingsEvent.new(settings)
    local self = PropertyBordersSettingsEvent.emptyNew()
    self.colorR = settings.color[1]
    self.colorG = settings.color[2]
    self.colorB = settings.color[3]
    self.colorA = settings.color[4]
    self.height = settings.height
    self.renderMode = settings.renderMode
    self.visible = settings.visible
    return self
end

function PropertyBordersSettingsEvent:readStream(streamId, connection)
    self.colorR = streamReadFloat32(streamId)
    self.colorG = streamReadFloat32(streamId)
    self.colorB = streamReadFloat32(streamId)
    self.colorA = streamReadFloat32(streamId)
    self.height = streamReadFloat32(streamId)
    self.renderMode = streamReadString(streamId)
    self.visible = streamReadBool(streamId)

    self:run(connection)
end

function PropertyBordersSettingsEvent:writeStream(streamId, connection)
    streamWriteFloat32(streamId, self.colorR)
    streamWriteFloat32(streamId, self.colorG)
    streamWriteFloat32(streamId, self.colorB)
    streamWriteFloat32(streamId, self.colorA)
    streamWriteFloat32(streamId, self.height)
    streamWriteString(streamId, self.renderMode)
    streamWriteBool(streamId, self.visible)
end

function PropertyBordersSettingsEvent:run(connection)
    -- Apply received settings
    if PropertyBorders ~= nil then
        local settings = PropertyBorders.settings
        settings.color = {self.colorR, self.colorG, self.colorB, self.colorA}
        settings.height = self.height
        settings.renderMode = self.renderMode
        settings.visible = self.visible

        -- Rebuild borders with new settings
        PropertyBorders:rebuildAllBorders()

        -- If server received from a client, broadcast to all other clients
        if not connection:getIsServer() then
            g_server:broadcastEvent(PropertyBordersSettingsEvent.new(settings), nil, connection, nil)
        end
    end
end
